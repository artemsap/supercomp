...
int main (void)
{
  ...
  int M = std::rand()%max_num + 1; //Number of columns in A matrix and rows in B matrix
  int N = std::rand()%max_num + 1; //Number of rows in A matrix
  int K = std::rand()%max_num + 1; //Number of columns in A matrix
  ...
  #pragma omp parallel num_threads(THREADS)
  {
    ...
    for(int i=0; i<num_col; i++)
      for(int j=0; j<num_rows; j++)
        matrix[i*num_rows +j] = (MIN_NUM + std::rand()%(MAX_NUM-MIN_NUM+1))/100.0;
  }
}
